...
function getMaxSidebarWidthPx() {
  if (typeof window === "undefined") {
    return SIDEBAR_WIDTH_MAX_PX
  }

  return Math.min(SIDEBAR_WIDTH_MAX_PX, window.innerWidth * 0.3)
}

function clampSidebarWidthPx(width: number) {
  return Math.round(
    Math.min(getMaxSidebarWidthPx(), Math.max(SIDEBAR_WIDTH_MIN_PX, width))
  )
}
...
function SidebarProvider({
  ...
  const [width, setWidthState] = React.useState(readStoredSidebarWidthPx)
  const [isResizing, setIsResizing] = React.useState(false)

  React.useEffect(() => {
    const handleWindowResize = () => {
      setWidthState((current) => clampSidebarWidthPx(current))
    }
  ...
  const setWidth = React.useCallback((nextWidth: number) => {
    const clamped = clampSidebarWidthPx(nextWidth)
...
function SidebarRail({ className, ...props }: React.ComponentProps<"button">) {
  ...
  return (
    <button
      ...
      data-resizing={isResizing ? "" : undefined}
      aria-label={
        state === "expanded" ? "Resize sidebar" : "Toggle sidebar"
      }
      tabIndex={-1}
      title={
        state === "expanded"
      ...
      className={cn(
        "absolute inset-y-0 z-20 hidden w-4 touch-none transition-all ease-linear group-data-[side=left]:-right-4 group-data-[side=right]:left-0 after:absolute after:inset-y-0 after:start-1/2 after:w-[2px] after:transition-colors hover:after:bg-sidebar-border sm:flex ltr:-translate-x-1/2 rtl:-translate-x-1/2",
        "in-data-[side=left]:cursor-col-resize in-data-[side=right]:cursor-col-resize",
        "[[data-side=left][data-state=collapsed]_&]:cursor-e-resize [[data-side=right][data-state=collapsed]_&]:cursor-w-resize",
        "data-[resizing]:after:bg-sidebar-border data-[resizing]:after:w-[3px]",
